...
def mask_path(edge_index: Tensor, p: float = 0.3, walks_per_node: int = 1,
    ...
    num_edges = edge_index.size(1)  # cora train edge 为例 8976 （总边数10556）

    edge_mask = edge_index.new_ones(num_edges, dtype=torch.bool)  #8976
    ...
    # rowptr操作其实就是传入csr矩阵的操作，这里用一个很简单的例子说说明一下
    # num_nodes=7
    # num_edges=7

    # edge_index = torch.tensor([[0,1,1,2,3,4,5],[1,0,2,3,4,5,6]])

    # edge_mask = edge_index.new_ones(num_edges, dtype=torch.bool)

    # row, col = edge_index
    # deg = degree(row, num_nodes=7)
    # rowptr = row.new_zeros(num_nodes + 1)
    # torch.cumsum(deg, 0, out=rowptr[1:])
    # n_id, e_id = random_walk(rowptr, col, torch.tensor([0,5]), 3, 1.0, 1.0)
    # print(n_id)
    # print(e_id)
    # print(e_id[e_id != -1].view(-1))
    # edge_mask[e_id] = False
    # print(edge_mask)
