...
class SquirrelDGL(torch.utils.data.Dataset):
    ...
    def __init__(self,args):
        ...
        # g = dgl.add_self_loop(g) 加载的文件中已经包含自环了，如果再加就两层自环了！还好自己的代码中对自环处理后再重加，惊险 不过chamelon和squirrel忘了加没加，到时候要判断一下
        self.graph = g
...
class SquirrelDataset(torch.utils.data.Dataset):
...
if __name__ == '__main__':
    dataset = SquirrelDataset('Squirrel')
